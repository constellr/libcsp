...
*/
#define PY_SSIZE_T_CLEAN // for python 3.11 and above
#include <Python.h>
#include <csp/csp.h>
...
static PyObject *pycsp_udp_init(PyObject *self, PyObject *args)
{
    ...
    int lport;
    int rport;
    if (!PyArg_ParseTuple(args, "sii", &host, &lport, &rport))
    {
        return NULL;
    }
    // csp_if_udp_conf_t conf = {.host = host, .lport = lport, .rport = rport};
    ...
    memset(udp_conf, 0, sizeof(csp_if_udp_conf_t));
    udp_conf->host = host;
