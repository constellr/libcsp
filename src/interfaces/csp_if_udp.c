...
static int csp_if_udp_tx(const csp_route_t *iroute, csp_packet_t *packet)
{

    csp_if_udp_conf_t *ifconf = iroute->iface->driver_data;
...
int csp_if_udp_rx_work(int sockfd, size_t unused, csp_iface_t *iface)
{
    ...
    csp_packet_t *packet = (csp_packet_t *)csp_buffer_get(0);
    // char data[100];
    if (packet == NULL)
    {
...
void *csp_if_udp_rx_loop(void *param)
{
    ...
    while (ifconf->sockfd == 0)
    {
        ...
        if (ifconf->sockfd < 0)
        {
            csp_log_info("  UDP server waiting for port %d\n", ifconf->lport);
            sleep(1);
            continue;
        }
        break;
...
void csp_if_udp_init(csp_iface_t *iface, csp_if_udp_conf_t *ifconf)
{
    ...
    {
        csp_log_warn("  Unknown peer address %s\n", ifconf->host);
    }
    csp_log_info("  UDP peer address: %s:%d (listening on port %d)\n", inet_ntoa(ifconf->peer_addr.sin_addr), ifconf->rport, ifconf->lport);
